...
ggplot(gapminder, 
  ...
  labs(title = "Life Expectancy over all continents",
         x = "Year",
         y = "Life Expectancy in years of age") + 
   NULL
```
...
Again recalling the original `gapminder` data frame from the gapminder package, we find that that data frame contains just six columns from the larger [data in Gapminder World](https://www.gapminder.org/data/). 

In this part, we join a few dataframes with more data than the 'gapminder' package. Specifically, we look at data on 
...
#create plot and facet by time period
